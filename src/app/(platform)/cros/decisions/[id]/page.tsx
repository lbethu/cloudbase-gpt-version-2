...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = requireVisible(viewer.identity, "decision", getRepositories().cros.decisions().find((r) => r.id === id));
  const related = relatedFor(viewer.identity, { type: "decision", id: record.id });
