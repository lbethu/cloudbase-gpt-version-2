...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = requireVisible(viewer.identity, "idea", getRepositories().cros.ideas().find((r) => r.id === id));
  const related = relatedFor(viewer.identity, { type: "idea", id: record.id });
