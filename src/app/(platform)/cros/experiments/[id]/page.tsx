...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = requireVisible(viewer.identity, "experiment", getRepositories().cros.experiments().find((r) => r.id === id));
  const related = relatedFor(viewer.identity, { type: "experiment", id: record.id });
