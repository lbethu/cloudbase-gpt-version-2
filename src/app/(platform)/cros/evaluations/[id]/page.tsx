...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = requireVisible(viewer.identity, "evaluation", getRepositories().cros.evaluations().find((r) => r.id === id));
  const related = relatedFor(viewer.identity, { type: "evaluation", id: record.id });
