...
export default async function IntegrationsPage() {
  ...
  const dbHost = cfg.database.url ? (() => { try { return new URL(cfg.database.url).host; } catch { return "configured"; } })() : "not configured";
  const rows: Array<{ name: string; status: "connected" | "configured" | "not configured" | "error" | "disabled"; detail: string; how: string }> = [
    ...
    { name: "Database (governed records)", status: cfg.storage.mode === "postgres" ? "connected" : "configured", detail: cfg.storage.mode === "postgres" ? `PostgreSQL · ${dbHost} · governed_records, imported_content, source_files, audit_events` : "Git-versioned file registry under content/ — no database required.", how: "Set DATABASE_URL (Neon and Supabase both have a free tier), then npm run db:migrate && npm run db:seed. CLOUDBASE_STORAGE=file forces the file registry." },
    { name: "Document storage", status: cfg.blob.mode === "s3" ? "connected" : "configured", detail: cfg.blob.mode === "s3" ? `S3-compatible bucket ${cfg.blob.bucket}` : `Local filesystem at ${cfg.sourceDocumentsDir} (never served from /public)`, how: "S3_BUCKET + S3_ENDPOINT/REGION/keys for AWS S3, Cloudflare R2, Supabase Storage or MinIO. Documents are always delivered through /api/files/{id} after an authorization check." },
    { name: "Audit sink", status: "connected", detail: cfg.storage.mode === "postgres" ? "audit_events table (append-only)" : `JSON lines in ${cfg.auditDir}`, how: "Follows the storage mode: JSONL in file mode, the audit_events table in postgres mode." },
  ];
