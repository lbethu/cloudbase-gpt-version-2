...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const TYPE = "account" as string;
  const found = (await crmAccounts(viewer.identity!)).items.find((r) => r.id === id);
  if (!found) notFound();
  ...
  const related = relatedFor(viewer.identity, { type: "account", id: record.id });
