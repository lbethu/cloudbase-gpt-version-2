...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const TYPE = "contact" as string;
  const found = (await crmContacts(viewer.identity!)).items.find((r) => r.id === id);
  if (!found) notFound();
  ...
  const related = relatedFor(viewer.identity, { type: "contact", id: record.id });
